...
const SEVERITY_BUTTON_STYLES: Record<number, { active: string; inactive: string; dot: string }> = {
  0: { active: "bg-gray-200 border-gray-500 text-gray-800 ring-2 ring-gray-500", inactive: "bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100", dot: "bg-gray-400" },
  1: { active: "bg-yellow-100 border-yellow-500 text-yellow-900 ring-2 ring-yellow-500", inactive: "bg-yellow-50 border-yellow-200 text-yellow-700 hover:bg-yellow-100", dot: "bg-yellow-500" },
  2: { active: "bg-orange-100 border-orange-500 text-orange-900 ring-2 ring-orange-500", inactive: "bg-orange-50 border-orange-200 text-orange-700 hover:bg-orange-100", dot: "bg-orange-500" },
  3: { active: "bg-red-100 border-red-500 text-red-900 ring-2 ring-red-500", inactive: "bg-red-50 border-red-200 text-red-700 hover:bg-red-100", dot: "bg-red-600" },
};
...
export default function FindingForm({ initialFinding, onSave }: Props) {
  ...
  const [heuristic, setHeuristic] = useState(initialFinding?.heuristic ?? HEURISTICS[0].label);
  const [severity, setSeverity] = useState<0 | 1 | 2 | 3>(initialFinding?.severity ?? 2);
  const [screen, setScreen] = useState(initialFinding?.screen ?? "");
  const [screenshot, setScreenshot] = useState<string | undefined>(initialFinding?.screenshot);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${errors.prototype ? "border-red-400" : "border-gray-300"}`}
        />
        <datalist id="prototype-options">
          {existingPrototypes.map((p) => <option key={p} value={p} />)}
        </datalist>
        {errors.prototype && <p className="mt-1 text-xs text-red-600">{errors.prototype}</p>}
      </div>

      {/* Heuristic */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Heuristic</label>
        <div className="relative">
          ...
          >
            {HEURISTICS.map((h) => <option key={h.id} value={h.label}>{h.label}</option>)}
          </select>
        ...
        {selectedHeuristic && (
          <p className="mt-1.5 text-xs text-gray-500 bg-gray-50 rounded-md px-3 py-2 border border-gray-100">{selectedHeuristic.description}</p>
        )}
      </div>
      ...
      <div>
          ...
          {([0, 1, 2, 3] as const).map((s) => {
            ...
            return (
              <button key={s} type="button" onClick={() => setSeverity(s)}
                className={`relative flex flex-col items-center gap-1 px-3 py-2.5 rounded-lg border text-sm font-medium transition-all ${isActive ? styles.active : styles.inactive}`}>
        ...
        <p className="mt-1.5 text-xs text-gray-500">{SEVERITY_DESCRIPTIONS[severity]}</p>
      </div>

      {/* Screen/Flow */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Screen / Flow</label>
        <input type="text" value={screen} onChange={(e) => setScreen(e.target.value)}
          placeholder="e.g. Checkout > Payment step"
          className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${errors.screen ? "border-red-400" : "border-gray-300"}`} />
        {errors.screen && <p className="mt-1 text-xs text-red-600">{errors.screen}</p>}
      </div>

      {/* Screenshot */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Screenshot <span className="font-normal text-gray-400">(optional)</span></label>
        <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center cursor-pointer hover:border-indigo-400 transition-colors"
          ...
          {screenshot ? (
            <div className="space-y-2">
              <img src={screenshot} alt="Screenshot preview" className="max-h-48 mx-auto rounded border border-gray-200" />
              <button type="button" onClick={(e) => { e.stopPropagation(); setScreenshot(undefined); if (fileInputRef.current) fileInputRef.current.value = ""; }}
                className="text-xs text-red-600 hover:text-red-800">Remove screenshot</button>
            </div>
          ) : (
      ...
      <div>
        ...
        <textarea value={observation} onChange={(e) => setObservation(e.target.value)}
          placeholder="What was observed? Keep this factual and objective." rows={3}
          className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none ${errors.observation ? "border-red-400" : "border-gray-300"}`} />
        {errors.observation && <p className="mt-1 text-xs text-red-600">{errors.observation}</p>}
      </div>
      ...
      <div>
        ...
        <textarea value={recommendation} onChange={(e) => setRecommendation(e.target.value)}
          placeholder="Suggested fix or improvement." rows={3}
          className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none ${errors.recommendation ? "border-red-400" : "border-gray-300"}`} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={source} onChange={(e) => setSource(e.target.value)} placeholder="e.g. Chris"
            className={`w-full rounded-md border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 ${errors.source ? "border-red-400" : "border-gray-300"}`} />
